...
# 🔒 [비밀번호] (Secret 기능 사용)
# ==========================================
# 만약 secrets 설정을 안 했다면 아래 줄을 지우고
# MY_PASSWORD = "7777" 이렇게 직접 적으세요.
try:
    MY_PASSWORD = st.secrets["password"]
except:
    MY_PASSWORD = "7777" # 비상용 기본 비번
...
st.title("🏀 NBA AI 승부사")
st.caption(f"📅 {datetime.now().strftime('%Y-%m-%d')} 데이터 분석")
...
if matches is None:
    ...
    st.success(f"✅ 데이터 로드 완료 ({date_str})")
    st.markdown("---")
    ...
    if st.button("🚀 분석 시작 (Click)", type="primary"):
        ...
        if not results:
            ...
            if len(results) >= 2:
                ...
                # 2. 멘트 설정
                if avg_score >= 80:
                    ment = "🌟 [초강력 추천] 자신감 Max! 금액 태워도 좋습니다."
                    box_color = "green"
                elif avg_score >= 70:
                    ment = "✅ [추천] 안정권입니다. 평소대로 가세요."
                    box_color = "blue"
                else:
                    ment = "🤔 [소액 도전] 리스크가 있습니다. 금액 조절하세요."
                    box_color = "yellow"
